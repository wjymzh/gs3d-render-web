...
class Camera extends Element {
  ...
  setDistance(distance: number, dampingFactorFactor: number = 1) {
    // clamp
    distance = Math.max(
      this.scene.config.controls.minZoom,
      Math.min(this.scene.config.controls.maxZoom, distance)
    );
  ...
  setOrientation(point: Vec3, dampingFactorFactor: number = 1.0) {
    ...
    const azim =
      Math.atan2(-point.x / distance, -point.z / distance) * math.RAD_TO_DEG;
  ...
  add() {
    ...
    if (this.scene.config.camera.debug_render) {
      this.entity.camera.setShaderPass(
        `debug_${this.scene.config.camera.debug_render}`
      );
    }
    ...
    this.mouseController.enableOrbit = this.touchController.enableOrbit =
      controls.enableRotate;
    this.mouseController.enablePan = this.touchController.enablePan =
      controls.enablePan;
    this.mouseController.enableZoom = this.touchController.enableZoom =
      controls.enableZoom;
  ...
  rebuildRenderTargets() {
    ...
    const samples = this.scene.config.camera.multisample
      ? device.maxSamples
      : 1;
  ...
  onUpdate(deltaTime: number) {
    ...
    if (config.controls.autoRotate) {
      if (this.autoRotateDelayValue > 0) {
        this.autoRotateDelayValue = Math.max(
          0,
          this.autoRotateDelayValue - deltaTime
        );
        this.autoRotateTimer = 0;
      } else {
        ...
        const rotateSpeed = Math.min(
          1,
          Math.pow(this.autoRotateTimer * 0.5 - 1, 5) + 1
        ); // soften the initial rotation speedup
        this.setAzimElev(
          this.azim +
            config.controls.autoRotateSpeed * 10 * deltaTime * rotateSpeed,
          this.elevation,
          0
        );
      }
    ...
    cameraPosition.mulScalar(
      this.focusDistance * (config.camera.dollyZoom ? distance.distance : 1.0)
    );
    ...
    this.entity.camera.fov =
      config.camera.fov * (config.camera.dollyZoom ? 1.0 : distance.distance);
  ...
  fitClippingPlanes(cameraPosition: Vec3, forwardVec: Vec3) {
    ...
    this.near = Math.max(
      0.001,
      dist < boundRadius ? this.far / 1024 : dist - boundRadius
    );
  }
  ...
  focus(options?: {
    ...
    const sceneRadius =
      options?.sceneRadius ?? this.scene.bound.halfExtents.length();
    const distance =
      sceneRadius / Math.sin(config.camera.fov * math.DEG_TO_RAD * 0.5);
