...
class MouseController {
  ...
  onDblClick = (event: globalThis.MouseEvent) => {
    ...
    const sx =
      (event.offsetX / target.clientWidth) * this.camera.scene.targetSize.width;
    const sy =
      (event.offsetY / target.clientHeight) *
      this.camera.scene.targetSize.height;

    this.camera.pickPrep(
      this.camera.scene.events.invoke("camera.mode") === "rings" ? 0.0 : 0.2
    );
    const pickId = this.camera.pick(sx, sy);

    if (pickId !== -1) {
      const splatPos = this.camera.scene.events.invoke(
      ...
      const res = this.camera.entity.camera.screenToWorld(
        event.offsetX,
        event.offsetY,
        1.0,
        vec
  ...
  orbit(dx: number, dy: number) {
    ...
    const azim =
      this.camera.azim -
      dx * this.camera.scene.config.controls.orbitSensitivity;
    const elev =
      this.camera.elevation -
      dy * this.camera.scene.config.controls.orbitSensitivity;
  ...
  pan(x: number, y: number) {
    ...
    const distance =
      this.camera.focusDistance * this.camera.distanceTween.value.distance;
...
class TouchController {
  ...
  onTouchMove(event: TouchEvent) {
    ...
    if (touches.length === 1 && this.enableOrbit) {
      ...
      const elev =
        this.camera.elevation -
        (touch.y - this.lastTouchPoint.y) *
          this.camera.scene.config.controls.orbitSensitivity;
      const azim =
        this.camera.azim -
        (touch.x - this.lastTouchPoint.x) *
          this.camera.scene.config.controls.orbitSensitivity;
      this.camera.setAzimElev(azim, elev);
