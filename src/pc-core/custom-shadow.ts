...
class CustomShadow extends Element {
  ...
  add() {
    ...
    this.renderTargets = this.textures.map(
      (texture) => new RenderTarget({ colorBuffer: texture, depth: true })
    );

    this.camera.camera.renderTarget = this.renderTargets[0];

    this.material.shader = createShader(
    ...
    this.shadowShader = createShader(
      this.scene.graphicsDevice,
      "Shadow",
      shadowDef
    );

    this.hblurShader = createShader(
      this.scene.graphicsDevice,
      "HBlur",
      hblurDef
    );
    this.vblurShader = createShader(
      this.scene.graphicsDevice,
      "VBlur",
      vblurDef
    );
  ...
  regenerate() {
    ...
    graphicsDevice.scope
      .resolve("sceneMin")
      .setValue([center.x - len, bound.getMin().y, center.z - len]);
    graphicsDevice.scope
      .resolve("sceneMax")
      .setValue([center.x + len, bound.getMax().y, center.z + len]);
  ...
  onPreRender() {
    this.material.setParameter(
      "shadowIntensity",
      this.scene.config.shadow.intensity
    );
  }
  ...
  onAdded(element: Element): void {
    if (
      ...
      modelElement.entity
        ...
        .forEach((render: RenderComponent) => {
