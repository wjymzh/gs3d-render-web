...
//           vec,
//           vec2,
//           Math.abs(Vec3.UP.dot(debugPlane)) > 0.99 ? Vec3.FORWARD : Vec3.UP
//         );
...
//     if (
//       editOp instanceof DeleteSelectionEditOp ||
//       editOp instanceof ResetEditOp
//     ) {
//       updateState(true);
//     }
//   });
...
//     scene.graphicsDevice.scope
//       .resolve("ringSize")
//       .setValue(mode === "rings" && events.invoke("splatSize") ? 0.04 : 0);
//     scene.forceRender = true;
//   });
...
//     scene.graphicsDevice.scope
//       .resolve("ringSize")
//       .setValue(events.invoke("camera.mode") === "rings" && value ? 0.04 : 0);
//     scene.forceRender = true;
//   });
...
//       const deletedPred = (i: number) =>
//         (state[i] & (State.hidden | State.deleted)) === 0;
//       const selectionPred = (i: number) =>
//         (state[i] & State.selected) === State.selected;
//       splatData.calcAabb(aabb, selectedSplats ? selectionPred : deletedPred);
//       splatData.calcFocalPoint(
//         vec,
//         selectedSplats ? selectionPred : deletedPred
//       );
...
//         focalPoint: vec,
//         distance:
//           (aabb.halfExtents.length() * vec2.x) /
//           scene.bound.halfExtents.length(),
//       });
//     }
//   });
...
//       const maxScale = Math.log(
//         Math.exp(scaleMin) + value * (Math.exp(scaleMax) - Math.exp(scaleMin))
//       );
...
//   events.on(
//     "select.byPlane",
//     (op: string, axis: number[], distance: number) => {
//       splatDefs.forEach((splatDef) => {
//         const splatData = splatDef.data;
//         const state = splatData.getProp("state") as Uint8Array;
//         const x = splatData.getProp("x");
...
//           data = convertPly(
//             splatDef.data,
//             splatDef.element.root.getWorldTransform()
//           );
//           extension = ".cleaned.ply";
//           break;
//         case "ply-compressed":
//           data = convertPlyCompressed(
//             splatDef.data,
//             splatDef.element.root.getWorldTransform()
//           );
...
//         splatDef.element.asset.file.filename
//       )}${extension}`;
