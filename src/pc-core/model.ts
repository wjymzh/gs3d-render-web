...
const calculateSortDistance = (
  drawCall: any,
  cameraPosition: Vec3,
  cameraForward: Vec3
) => {
  return vec
    .sub2(drawCall.node.getPosition(), cameraPosition)
    .dot(cameraForward);
};
...
class Model extends Element {
  ...
  add() {
    ...
    this.entity
      ...
      .forEach((component: RenderComponent) => {
        if (component.entity.enabled) {
          this.scene.shadowLayer.shadowCasters =
            this.scene.shadowLayer.shadowCasters.concat(
  ...
  remove() {
    this.entity
      ...
      .forEach((component: RenderComponent) => {
        this.scene.shadowLayer.shadowCasters =
          this.scene.shadowLayer.shadowCasters.filter(
